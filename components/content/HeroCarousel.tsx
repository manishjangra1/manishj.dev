...
export function HeroCarousel({
  ...
  return (
    ...
    >
      ...
      <div
        className="relative w-full h-[290px] sm:h-[310px] md:h-[320px] xl:h-[330px] flex items-center justify-center [perspective:1000px] overflow-visible py-1"
            ...
            <div
              ...
              style={{
                transform: isCenter
                  ...
                  : isLeft
                  ? 'translateX(-48%) scale(0.85) translateZ(-30px) rotateY(12deg)'
                  : 'translateX(48%) scale(0.85) translateZ(-30px) rotateY(-12deg)',
                zIndex: isCenter ? 20 : 10,
                opacity: isCenter ? 1 : 0.45,
              }}
              className={cn(
                'absolute top-0 w-[210px] sm:w-[230px] md:w-[245px] xl:w-[265px] rounded-none bg-[var(--color-card)] border border-[var(--color-border)] p-3 sm:p-3.5 flex flex-col justify-between cursor-pointer',
            ...
            >
              ...
              <div className="relative w-full aspect-[16/10] rounded-none overflow-hidden bg-[var(--color-surface)] border border-[var(--color-border)]">
                ...
                {project.liveUrl && (
                  <a
                    ...
                    className="absolute top-1.5 right-1.5 w-5 h-5 rounded-none bg-black/80 backdrop-blur-md border border-white/20 flex items-center justify-center text-white hover:bg-black transition-all z-10"
                  >
              ...
              <div className="flex flex-col mt-2 sm:mt-2.5">
                <div className="flex items-center justify-between gap-1">
                  <span className="font-mono text-[8px] sm:text-[9px] font-semibold uppercase tracking-wider text-[var(--color-text-muted)] truncate">
                    {project.kicker || 'FEATURED PROJECT'}
                  </span>
                  <span className="font-mono text-[8px] sm:text-[8.5px] text-[var(--color-text-muted)]">
                    #{String(index + 1).padStart(2, '0')}
                  </span>
                </div>

                <h3 className="text-[13px] sm:text-[14px] font-bold text-[var(--color-text)] tracking-tight mt-0.5 truncate">
                  {project.title}
                </h3>
                <p className="text-[10px] sm:text-[11px] leading-[1.35] text-[var(--color-text-secondary)] mt-1 line-clamp-2">
                  {project.lede}
                </p>
                ...
                {project.meta && project.meta.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {project.meta.slice(0, 3).map((tech) => (
                      <span
                        key={tech}
                        className="px-1.5 py-0.5 rounded-none bg-[var(--color-surface)] border border-[var(--color-border)] text-[8px] font-mono text-[var(--color-text-secondary)]"
                      >
                        {tech}
                      </span>
                    ))}
                ...
                <div className="mt-2 sm:mt-2.5 pt-1.5 border-t border-[var(--color-border)] flex items-center justify-between">
                  <Link
                    href={`/work/${project.slug}`}
                    className="inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-mono font-medium text-[var(--color-text)] hover:text-white transition-colors group/link"
                  >
        ...
        {total > 1 && (
          <>
            ...
            <button
              ...
              className="absolute -right-3 sm:-right-4 md:-right-6 lg:-right-6 top-1/2 -translate-y-1/2 z-30 w-7 h-7 sm:w-8 sm:h-8 rounded-none bg-[var(--color-surface)] hover:bg-[var(--color-surface-hover)] border border-[var(--color-border-strong)] text-[var(--color-text)] flex items-center justify-center shadow-xl transition-all duration-150 hover:scale-105 active:scale-95 text-xs font-mono"
            >
              <span>›</span>
            </button>
          </>
